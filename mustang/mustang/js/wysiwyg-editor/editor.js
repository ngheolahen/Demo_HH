	...
	var methods = {
			...
			var getURL = $("<button/>",{
				...
				var image = $('<img/>',{
					src:url,
						...
						return false;
				}).load( function() { $(this).appendTo(a).on('click',function(){
					$('#imageList').data('current', $(this).attr('src'));
				});
				li.append(a).appendTo($('#imageList'));
			});
							 ...
							 'font_size': {	"select":true,
								...
								'color'	: { "text":"A",
											...
											"custom":function(button){
													var editor = $(this);
													var flag = 0;
													var paletteCntr   = $('<div/>',{id:"paletteCntr",class:"activeColour", css :{"display":"none","width":"335px"}}).on('click',function(event){event.stopPropagation();});
													...
													if(flag==0){
														for (var i = 0; i < colors.length; i++){
															if(colors[i].hex!=null){
																	palette.append($('<li />').css('background-color', colors[i].hex).mousedown(function(event){ event.preventDefault();}).on('click',function(){															
																...
																bgPallette.append($('<li />').css('background-color', colors[i].hex).mousedown(function(event){ event.preventDefault();}).on('click',function(){															
																var hexcolor = methods.rgbToHex.apply(this,[$(this).css('background-color')]);
																methods.restoreSelection.apply(this);
																methods.setStyleWithCSS.apply(this);
						...
						'insert_table'	: { "modal": true,
							...
							'splchars'	: { "text": "S", 
											...
											"custom":function(button){
													...
													var flag =0;
													var splCharDiv = $('<div/>',{id:"specialchar", class:"specialCntr", css :{"display":"none"}}).on('click',function(event) { event.stopPropagation();});
													...
													if(flag==0){
														for (var i = 0; i < specialchars.length; i++){															
															splCharUi.append($('<li />').html(specialchars[i].text).attr('title',specialchars[i].name).mousedown(function(event){ event.preventDefault();}).on('click',function(event){
																if(navigator.userAgent.match(/MSIE/i)){
		...
		createImageContext: function(event,cMenuUl){
			...
			cMenuUl.append($('<li/>').append(modalTrigger))
					.append($('<li/>').append($('<a/>',{text:"Remove Image"}).on('click', 
		...
		createTableContext: function(event,cMenuUl){
			...
			var modalId="editProperties";
				...
				$(this).data("editor").focus();
					...
					var modalTrigger = $('<a/>',{	href:"#"+modalId,
											...
											"data-toggle":"modal"
			}).on('click', function(e){ return function(){	
							$('#context-menu').remove();			
			...
			cMenuUl.append($('<li/>',{class:"dropdown-submenu",css:{display:"block"}})
									...
									.append($('<ul/>',{class:"dropdown-menu"})
											...
											.append($('<li/>').append($('<a/>',{text:"Remove Row"}).on('click', 
											function(e) { return function(){ 
													$('#context-menu').remove();
							...
							.append($('<ul/>',{class:"dropdown-menu"})
									...
									.append($('<li/>').append($('<a/>',{text:"Remove Column"}).on('click', 
										function(e) { return function(){ 
												$('#context-menu').remove();
												var selectedCell = $(e.target);
												var columnIndex = selectedCell.siblings().addBack().index(selectedCell);
			...
			cMenuUl.append($('<li/>').append(modalTrigger))
					...
					.append($('<li/>').append($('<a/>',{text:"Remove Table"}).on('click', 
						function(e){ return function(){ 
								$('#context-menu').remove();
															...
															class:"modal-dialog"
																...
																class:"modal-content"
																	...
																	class:"modal-footer"
																			...
																			e.preventDefault();
																			}).on('click',function(obj){return function(){onSave.apply(obj)}}(this)))
																)
														)	
												);	
